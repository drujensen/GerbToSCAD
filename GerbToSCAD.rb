...
output_filename = ARGV[1] if ARGV.length > 1
if ARGV.length > 0
...
File.open(input_filename, 'r') do |input_file|
  while (line = input_file.gets)
    ...
    # Look for moves
    next unless data = /X(\d+)Y(\d+)D02/.match(line)
...
output_file.write("    block (#{x}, #{y}, board_thickness);\n")
output_file.write("\n")
...
output_file.write("    stand (#{x}, #{y}, board_thickness);\n")
output_file.write("\n")
